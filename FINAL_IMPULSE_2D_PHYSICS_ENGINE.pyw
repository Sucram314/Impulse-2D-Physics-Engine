...
class Camera:
    ...
    def update(self,mx,my,worldPoint,keys,scrolling,shift,dt):
    ...
    def toCameraSpace(self,coord):
        return [(coord[0]-self.x)*self.m + self.offsetX,(self.y-coord[1])*self.m + self.offsetY]

    def toWorldSpace(self,coord):
        return [(coord[0] - self.offsetX)/self.m + self.x,-(coord[1] - self.offsetY)/self.m + self.y]

def draw_polygon(surface,colour,points):
    try:
        p = [scene.camera.toCameraSpace(point) for point in points]
...
def draw_circle(surface,colour,x,y,r):
    try:
        pos = scene.camera.toCameraSpace((x,y))
        l = r*scene.camera.m
...
def draw_point(surface,colour,point):
    try:
        pygame.draw.circle(surface,colour,scene.camera.toCameraSpace(point),5)
    except:
        pass

def draw_line(surface,colour,p1,p2):
    try:
        pygame.draw.aaline(surface,colour,scene.camera.toCameraSpace(p1),scene.camera.toCameraSpace(p2))
    except:
        pass

def draw_rect(surface,colour,p1,p2):
    try:
        a = scene.camera.toCameraSpace(p1)
        b = scene.camera.toCameraSpace(p2)
...
class Scene:
        ...
        if not self.paused:
            for i in range(len(self.objs)-1):
                ...
                for j in range(i+1,len(self.objs)):
                    ...
                    if A.invMass + B.invMass == 0 or A.layer & B.layer == 0 or (i,j) in self.noCollisionPairs: continue
    ...
    def draw(self):
        ...
        for beam in self.beams: beam.draw()
...
class Collision:
    ...
    def resolve(self,slop=0.0198,percentage=0.25):
        ...
        for contact in self.contacts:
            if DEBUGMODE: draw_point(screen,(255,0,0),contact)
...
class NoCollisionPair:
    def __init__(self,A,B):
        ...
        scene.noCollisionPairs.add((self.idxA,self.idxB))

    def close(self):
        scene.noCollisionPairs.remove((self.idxA,self.idxB))
...
class Circle:
    ...
    def draw(self):
        ...
        if DEBUGMODE: draw_rect(screen,(255,0,0),(self.AABB[0],self.AABB[1]),(self.AABB[2],self.AABB[3]))
    ...
    def collide(self,obj):
        if type(obj) == Circle:
            if not AABBAABBCollide(self.AABB,obj.AABB): return
...
class Plane:
    ...
    def draw(self):
        pos = scene.camera.toCameraSpace((self.x,self.y))
...
class Spring:
    ...
    def draw(self,col=(255,255,255)):
        p1 = rotate(self.p1,self.A.a)
        p2 = rotate(self.p2,self.B.a)
        p1 = scene.camera.toCameraSpace((p1[0]+self.A.x,p1[1]+self.A.y))
        p2 = scene.camera.toCameraSpace((p2[0]+self.B.x,p2[1]+self.B.y))
        p1 = (int(p1[0]),int(p1[1]))
        p2 = (int(p2[0]),int(p2[1]))
        pygame.draw.line(screen,col,p1,p2,max(1,int(scene.camera.m*4)))
        pygame.draw.circle(screen,col,p1,max(1,int(scene.camera.m*5)))
        pygame.draw.circle(screen,col,p2,max(1,int(scene.camera.m*5)))
...
class Beam:
    def __init__(self,A,B,p1,p2,l=None):
        ...
        if self.invMassSum != 0:
            ...
            self.BinvInertiaPortion = self.invInertiaSum - self.AinvInertiaPortion

        self.ogA = self.A.a
        self.relA = self.A.a-self.B.a

        scene.beams.append(self)

    def draw(self,col=(127,0,255)):
        p1 = rotate(self.p1,self.A.a)
        p2 = rotate(self.p2,self.B.a)
        p1 = scene.camera.toCameraSpace((p1[0]+self.A.x,p1[1]+self.A.y))
        p2 = scene.camera.toCameraSpace((p2[0]+self.B.x,p2[1]+self.B.y))
...
class Fixjoint:
    def __init__(self,A,B,p1,p2):
        ...
        self.noCollsionPair = NoCollisionPair(self.A,self.B)
    ...
    def update(self,percentage=0.25):
        ...
        relA = self.relA-self.A.a-self.A.av+self.B.a+self.B.av
        ...
        if self.A.invMass != 0:
            ...
            self.A.a += relA*self.AinvInertiaPortion
        ...
        if self.B.invMass != 0:
            ...
            self.B.a -= relA*self.BinvInertiaPortion
...
Circle(100,-1800,1000,0)
Hinge(scene.objs[-3],scene.objs[-2],(0,0),(0,0))
Hinge(scene.objs[-2],scene.objs[-1],(0,0),(0,0))

Polygon([(-500,-50),(500,-50),(500,50),(-500,50)],-3000,4000,0)
Polygon([(-500,-50),(500,-50),(500,50),(-500,50)],-3000,4000,90)
Axle(scene.objs[-2],scene.objs[-1],(0,0),(0,0))
...
while 1:
    ...
    worldPoint = scene.camera.toWorldSpace((mx,my))
    ...
    scene.update()
    scene.camera.update(mx,my,worldPoint,keys,scrolling,shift,dt)
    scene.draw()
